    ...
    (->> views
         (map-indexed (fn [view-idx view]
                        (let [x-view (into {} (filter #(= view-idx
                                                          (get view-assignments (first %)))
                                                      x))]
                          (log-likelihood-view x-view row-id types (get-in latents [:local view-idx]) view))))
         (reduce +))))
...
(defn log-likelihood
  ...
  [data model latents]
  (let [view-assignments (get-in latents [:global :z])
        data-formatted   (->> data
...
(defn view-category-weights
  "latents must be view latents, constraints must pertain to the view."
...
(defn sample-category
  "Given weights and a list of categories of equal length,
  samples a category."
  [weights categories]

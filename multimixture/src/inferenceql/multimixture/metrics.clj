(ns inferenceql.multimixture.metrics
...
(defn jensen-shannon-divergence
  ...
  Since it depends on `inferenceql.multimixture.metrics/kl-divergence`, 
  `p` must be zero when `q` is zero. `p` and `q` must have the same
  alphabet size (i.e. length)."
  [p q]
  (check-distribution-criteria p q)

...
=== `inferdb.search_by_example.search.columns`
...
Takes a hypothetical row and returns an array of pair where the first element is the index of the row in `inferdb.search_by_example.search.data` and the second element is the similarity score for that row.

  ...
  (let [run (atom 0)
                         ...
                         (doall (for [[dataset-name gen-fn] (seq generators)]
                                  (let [datum (gen-fn)
                                    ...
                                    (.. vega-instance
                                        ...
                                        (resize)
                                        (run))))))
        embed (fn [this spec opt generators pts-store init-fn]
    ...
    (r/create-class
     {:display-name "vega-lite"
      ...
      :component-did-mount
      (fn [this]
        (embed this spec opt generators pts-store (:init-fn options))
      ...
      (fn [this old-argv]
        (let [[_ old-spec old-opt old-generators _old-pts-store _old-options] old-argv
          ...
          (when (not= [old-spec old-opt old-generators]
            ...
            (embed this new-spec new-opt new-generators current-pts-store (:init-fn new-options)))))
